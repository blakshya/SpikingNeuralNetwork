...
def train_model(arg1):
	global n_input, n_output,target,lastspike_pre,lastspike_post
	global TIME_STEP, SINGE_SAMPLE_TIME,time_index,V_TH,V_RESET,TAU_M_EXCI,TAU_UP,TAU_DOWN,TAU_M_INH,TAU_S,SYNAP_PULSE_DURATION,I_BIAS__used,W_MAX,INHIBIT_WEIGHT_MATRIX,pre_spiking_log,post_spiking_log,SYNAP_CURR_CONS_EXC,SYNAP_CURR_CONS_INH,A_UP,A_DOWN
	[x_train,y_train,x_test,y_test,I_BIAS,SYNAP_CURR_CONS_EXC,SYNAP_CURR_CONS_INH,A_UP,A_DOWN,epochs,n_input,n_output] = arg1
	pass
	#-----------------------------------------------------------------------------
	# Constants 
	#-----------------------------------------------------------------------------
	...
	while j < num_examples :
		...
		while k < SINGE_SAMPLE_TIME :
			...
			update_time()
			k += 1
		class_prob = np.sum(spike_countlog_post,axis=1)  #/ len(np.nonzero(np.any(		predicted_class = (class_prob == np.amax(class_prob))

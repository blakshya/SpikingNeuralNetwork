...
''' 
This code is for partially suervised learning
It always give 100% train accuracy
The test accuracy varies around 90-94% which is decent for training on a single epoch
'''
'''
The graphs are provided for better visualisation of data. Turning them off has no consequence on the correctness of the working. Turning them off speeds up the execution tremendously
'''
#-----------------------------------------------------------------------------
# Functions
##-----------------------------------------------------------------------------
...
def fig_update(fig1,weightMatrix,wt_pdate,pre_spiking_log,post_spiking_log,lastspike_pre,lastspike_post,neurons_ip,neurons_op):
	...
	ax_pre_time.set_title('Time since input neuron has fired last')
	...
	ax_post_time.set_title('Time since output neuron has fired last')
	ax_post_time.set_xticks([])
	ax_post_time.set_yticks([])
	for i in range(n_output):
		if lastspike_post[i,0] < 500:
	...
	ax_nin.set_title('Input neuron voltage (mV)')
	im_nin = ax_nin.imshow(neurons_ip.T)
	for i in range(n_input):
		text = ax_nin.text(i,0,'%1.0f'%(1e3*neurons_ip[i,0]),ha='center',
				va='center',color='w')
	ax_nin.set_yticks([])
	...
	ax_nout.set_title('Output neuron voltage (mV)')
	im_nout = ax_nout.imshow(neurons_op.T)
	ax_nout.set_yticks([])
	ax_nout.set_xticks(np.arange(n_output))
	for i in range(n_output):
		text = ax_nout.text(i,0,'%1.0f'%(1e3*neurons_op[i,0]),ha='center',
			va='center',color='w')
...
SYNAP_CURR_CONS_EXC = 4e-4 #nA
SYNAP_CURR_CONS_INH = 2e-3 #nA
...
# ax1.plot(np.arange(len(train_acc_list)),train_acc_list)
